...
# --- 全局配置 ---
PYNQ_IP = "192.168.2.99"  # <--- 请确保 IP 正确
...
class FFTWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PYNQ-Z2 Audio Analyzer")
    ...
    def init_immersion_mode_ui(self):
        ...
        center_png = os.path.join(script_dir, "center.png")
        center_jpg = os.path.join(script_dir, "center.jpg")

        if os.path.exists(center_png) or os.path.exists(center_jpg):
            fpath = center_png if os.path.exists(center_png) else center_jpg
