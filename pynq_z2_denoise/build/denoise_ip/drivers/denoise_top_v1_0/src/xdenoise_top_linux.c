...
typedef struct {
    ...
    XDenoise_top_uio_map maps[ MAX_UIO_MAPS ];
} XDenoise_top_uio_info;

/***************** Variable Definitions **************************************/
static XDenoise_top_uio_info uio_info;
...
static int uio_info_read_name(XDenoise_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDenoise_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDenoise_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDenoise_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDenoise_top_Initialize(XDenoise_top *InstancePtr, const char* InstanceName) {
	XDenoise_top_uio_info *InfoPtr = &uio_info;
