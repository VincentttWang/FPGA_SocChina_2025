...
entity denoise_top_fifo_w24_d16_S is 
    generic (
...
architecture rtl of denoise_top_fifo_w24_d16_S is
------------------------Task and function--------------
------------------------Parameter----------------------
constant SRL_DEPTH  : INTEGER := DEPTH;
constant SRL_AWIDTH : INTEGER := ADDR_WIDTH;
------------------------Component----------------------
    component denoise_top_fifo_w24_d16_S_ShiftReg is
    generic (
        DATA_WIDTH : integer := 24;
        ADDR_WIDTH : integer := 4;
        DEPTH      : integer := 16);
...
begin
----------------------- Instantiation -----------------------
    U_denoise_top_fifo_w24_d16_S_ShiftReg : denoise_top_fifo_w24_d16_S_ShiftReg
    generic map (
...
--------------------------- Body ----------------------------
    ...
    process (clk, reset) begin
        ...
        if reset = '1' then
            ...
            if push = '1' and pop = '0' and mOutPtr = DEPTH - 1 then
                full_n <= '0';
            elsif push = '0' and pop = '1' then
                full_n <= '1';
            end if;
...
entity denoise_top_fifo_w24_d16_S_ShiftReg is
    generic (
        DATA_WIDTH : integer := 24;
        ADDR_WIDTH : integer := 4;
        DEPTH      : integer := 16);
    port (
        ...
        din        : in std_logic_vector(DATA_WIDTH-1 downto 0);
        dout       : out std_logic_vector(DATA_WIDTH-1 downto 0));
end denoise_top_fifo_w24_d16_S_ShiftReg;

architecture rtl of denoise_top_fifo_w24_d16_S_ShiftReg is
