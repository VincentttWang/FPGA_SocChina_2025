...
# --- 全局配置 ---
PYNQ_IP = "192.168.137.177"  # <--- 请确保 IP 正确
...
class FFTWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn_analyze = QPushButton("智能音频分析 🎵")
    ...
    def start_local_mic(self, device_idx):
        if self.audio_worker:
            self.audio_worker.stop()
        # Mic 增益 15.0
        self.audio_worker = AudioWorker(device_id=device_idx, sample_rate=self.SAMPLING_RATE, digital_gain=15.0)
    ...
    def init_immersion_mode_ui(self):
        ...
        if os.path.exists("center.png") or os.path.exists("center.jpg"):
            ...
            center_pix = center_pix.scaled(200, 200, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            center_item = QGraphicsPixmapItem(center_pix)
            center_item.setPos(-center_pix.width()/2, -center_pix.height()/2)
            center_item.setZValue(10)
            self.immer_plot.addItem(center_item)
    ...
    def finalize_analysis(self):
        ...
        idx_bass_end = int(300 / self.freq_scale)    
        idx_mid_end = int(3400 / self.freq_scale)    
        ...
        if energy_total < 100: 
            ...
            msg_box.setInformativeText(
                ...
                f"低频占比 (<300Hz): {ratio_bass:.1%}\n"
                f"中频占比 (人声段): {ratio_mid:.1%}\n"
                f"高频占比 (>3.4kHz): {ratio_high:.1%}\n"
                f"详细推断: {detail}"
            )
